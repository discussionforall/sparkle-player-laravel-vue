...
class MediaScannerTest extends TestCase
{
    ...
    public function scan(): void
    {
        ...
        self::assertDatabaseHas(Artist::class, ['name' => 'Sparkle']);

        // Albums should be created
        self::assertDatabaseHas(Album::class, ['name' => 'Sparkle Testing Vol. 1']);

        // Albums and artists should be correctly linked
        /** @var Album $album */
        $album = Album::query()->where('name', 'Sparkle Testing Vol. 1')->first();
        self::assertSame('Sparkle', $album->artist->name);
        ...
        self::assertSame('Sparkle', $song->album_artist->name);
    ...
    public function optionallyIgnoreHiddenFiles(): void
    {
        ...
        config(['sparkle.ignore_dot_files' => false]);
        $this->scanner->scan($config);
        self::assertDatabaseHas(Album::class, ['name' => 'Hidden Album']);

        config(['sparkle.ignore_dot_files' => true]);
        $this->scanner->scan($config);
