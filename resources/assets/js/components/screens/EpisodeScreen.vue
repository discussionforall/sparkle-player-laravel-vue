<template>
  <ScreenBase @contextmenu.prevent="onContextMenu">
      ...
      <ScreenHeader v-if="episode">
        ...
        <template #controls>
          <div class="flex gap-2">
            <Btn v-sparkle-tooltip="playing ? 'Pause' : 'Play'" highlight @click.prevent="playOrPause">
              <Icon v-if="playing" :icon="faPause" fixed-width />
              <Icon v-else :icon="faPlay" fixed-width />
            </Btn>

            <Btn v-sparkle-tooltip="'Download'" gray @click.prevent="download">
              <Icon :icon="faDownload" fixed-width />
            </Btn>

            <Btn
              v-if="episode.episode_link"
              v-sparkle-tooltip="'Visit episode webpage'" :href="episode.episode_link"
              gray
              tag="a"
              target="_blank"
            >
    ...
    <div v-if="episode">
      ...
      <div v-sparkle-new-tab class="description text-k-text-secondary" v-html="formattedDescription" />
    </div>
  </ScreenBase>
</template>
