...
new class extends UnitTestCase {
  protected test () {
    ...
    it('shows About Sparkle model', async () => {
      const emitMock = this.mock(eventBus, 'emit')
      this.renderComponent()

      await this.user.click(screen.getByRole('button', { name: 'About Sparkle' }))

      expect(emitMock).toHaveBeenCalledWith('MODAL_SHOW_ABOUT_SPARKLE')
    })
