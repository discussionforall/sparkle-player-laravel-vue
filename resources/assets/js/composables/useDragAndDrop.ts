...
const getDragType = (event: DragEvent) => {
  return draggableTypes.find(type => event.dataTransfer?.types.includes(`application/x-sparkle.${type}`))
}
...
export const useDraggable = (type: DraggableType) => {
  const startDragging = (event: DragEvent, dragged: Draggable) => {
    ...
    event.dataTransfer.setData(`application/x-sparkle.${type}`, JSON.stringify(data))
...
export const useDroppable = (acceptedTypes: DraggableType[]) => {
  ...
  const getDroppedData = (event: DragEvent) => {
    ...
    try {
      return JSON.parse(event.dataTransfer!.getData(`application/x-sparkle.${type}`)!)
    } catch (error: unknown) {
  ...
  const resolveDroppedValue = async <T = Playlist> (event: DragEvent): Promise<T | undefined> => {
    try {
      switch (getDragType(event)) {
        case 'playlist':
          const id = String(JSON.parse(event.dataTransfer!.getData('application/x-sparkle.playlist')))
