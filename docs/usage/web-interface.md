...
Sparkle’s web interface is designed to be as simple and intuitive as possible, drawing inspiration from the best music players out there. Here’s a quick rundown of what you can do with it.

## Browsing and Searching for Music

The navigation bar on the left side of the screen is where you can find all the main features of Sparkle.
...
Sparkle supports creating an unlimited number of playlists as well as organizing them into folders.
...
and Sparkle will automatically update the playlist based on those criteria.
...
Sparkle will include in the playlist:
...
As an admin (Community edition) or song owner (Sparkle Plus), you can edit a song's metadata by right-clicking on it and choosing "Edit…".
...
As an admin (Community edition) or song owner (Sparkle Plus), you can delete a song by right-clicking on it and choosing "Delete from Filesystem."
...
By default, Sparkle keeps a backup of the song before deleting it.
...
This will copy a URL to the clipboard that can then be shared with anyone with a Sparkle account.

## Keyboard Shortcuts

Sparkle has a number of keyboard shortcuts to make it easier to navigate and control playback.
...
With a browser that supports [MediaSession](https://developer.mozilla.org/en-US/docs/Web/API/MediaSession) — basically, all modern browsers — you can control Sparkle using hardware media keys without even having Sparkle _or the browser_ focused.
