...
{
    protected $signature = 'sparkle:storage:s3';
    protected $description = 'Set up Amazon S3 or a compatible service as the storage driver for Sparkle';
    ...
    {
        if (!License::isPlus()) {
            $this->components->error('S3 as a storage driver is only available in Sparkle Plus.');

            return self::FAILURE;
        }

        $this->components->info('Setting up S3 or an S3-compatible service as the storage driver for Sparkle.');
