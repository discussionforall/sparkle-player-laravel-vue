...
{
    protected $signature = 'sparkle:storage:dropbox';
    protected $description = 'Set up Dropbox as the storage driver for Sparkle';
    ...
    {
        if (!License::isPlus()) {
            $this->components->error('Dropbox as a storage driver is only available in Sparkle Plus.');

            return self::FAILURE;
        }

        $this->components->info('Setting up Dropbox as the storage driver for Sparkle.');
        ...
        $this->comment('Please visit the following link to authorize Sparkle to access your Dropbox account:');
        $this->info($tmpUrl);
        $this->comment('The link will expire in 15 minutes.');
        $this->comment('After you have authorized Sparkle, enter the access code below.');
