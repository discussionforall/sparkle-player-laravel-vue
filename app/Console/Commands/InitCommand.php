...
class InitCommand extends Command
{
    ...
    private const DEFAULT_ADMIN_NAME = 'Sparkle';
    private const DEFAULT_ADMIN_EMAIL = 'admin@sparkle.dev';
    private const DEFAULT_ADMIN_PASSWORD = 'SparkleIsCool';
    private const NON_INTERACTION_MAX_DATABASE_ATTEMPT_COUNT = 10;

    protected $signature =
        'sparkle:init {--no-assets : Do not compile front-end assets} {--no-scheduler : Do not install scheduler}';
    protected $description = 'Install or upgrade Sparkle';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $this->components->error("Oops! Sparkle installation or upgrade didn't finish successfully.");
            $this->components->error('Please check the error log at storage/logs/laravel.log and try again.');
            $this->components->error('For further troubleshooting, visit https://docs.sparkle.dev/troubleshooting.');
        ...
        $this->info('Sparkle can now be run from localhost with `php artisan serve`.');
        ...
        if (!Setting::get('media_path')) {
            $this->info('You can set up the storage with `php artisan sparkle:storage`.');
        }

        $this->info('Again, visit 📙 ' . config('sparkle.misc.docs_url') . ' for more tips and tweaks.');

        $this->info(
            "Feeling generous and want to support Sparkle’s development? Check out "
            . config('sparkle.misc.sponsor_github_url')
            . ' 🤗'
        );

        $this->info('Thanks for using Sparkle. You rock! 🤘');
    ...
    private function setUpDatabase(): void
    {
        ...
        if ($config['DB_CONNECTION'] === 'sqlite-e2e') {
            ...
            $config['DB_DATABASE'] = $this->anticipate('DB name', ['sparkle']);
            $config['DB_USERNAME'] = $this->anticipate('DB user', ['sparkle']);
    ...
    private function maybeSetMediaPath(): void
    {
        ...
        $this->info('If you plan to use Sparkle with a cloud provider (S3 or Dropbox), you can also skip this.');

        while (true) {
            $path = $this->ask('Media path', config('sparkle.media_path'));
    ...
    {
        $path = config('sparkle.media_path');
    ...
    private function tryInstallingScheduler(): void
    {
        ...
        $this->components->info('Trying to install Sparkle scheduler…');

        if (Artisan::call('sparkle:scheduler:install') !== self::SUCCESS) {
            $this->components->warn(
                'Failed to install scheduler. ' .
                'Please install manually: https://docs.sparkle.dev/cli-commands#command-scheduling'
            );
        } else {
            $this->components->info('Sparkle scheduler installed successfully.');
        }
    }
}
