...
{
    protected $signature = 'sparkle:doctor';
    protected $description = 'Check Sparkle setup';
    ...
    public function handle(): int
    {
        ...
        $this->components->alert('Checking Sparkle setup...');
        $this->line('');
        ...
        if ($this->errors) {
            ...
            $this->output->success('Your Sparkle setup should be good to go!');
        }
    ...
    {
        $this->components->error('There are errors in your Sparkle setup. Sparkle will not work properly.');

        if (File::isWritable(base_path('storage/logs/laravel.log'))) {
            /** @var Throwable $error */
            foreach ($this->errors as $error) {
                Log::error('[SPARKLE.DOCTOR] ' . $error->getMessage(), ['error' => $error]);
            }
    ...
    private function checkPlusLicense(): void
    {
        try {
            ...
            if ($status->hasNoLicense()) {
                $this->reportInfo('Sparkle Plus license status', 'Not available');

                return;
            }

            if ($status->isValid()) {
                $this->reportSuccess('Sparkle Plus license status', 'Active');
            } else {
                $this->reportError('Sparkle Plus license status', 'Invalid');
            }
        } catch (Throwable $e) {
            $this->collectError($e);
            $this->reportWarning('Cannot check for Sparkle Plus license status');
        }
    }
    ...
    private function checkScheduler(): void
    {
        ...
        if (InstallSchedulerCommand::schedulerInstalled($crontab)) {
            $this->reportSuccess('Sparkle scheduler status', 'Installed');
        } else {
            $this->reportWarning('Sparkle scheduler status', 'Not installed');
        }
    }
    ...
    {
        $this->reportInfo('Streaming method', config('sparkle.streaming.method'));
    }
    ...
    {
        $ffmpegPath = config('sparkle.streaming.ffmpeg_path');
