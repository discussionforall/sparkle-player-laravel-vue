...
class MediaScanner
{
    ...
    private function gatherFiles(string $path): array
    {
        return iterator_to_array(
            $this->finder->create()
                ...
                ->ignoreDotFiles((bool) config('sparkle.ignore_dot_files'))
                ->files()
                ->followLinks()
    ...
    private function setSystemRequirements(): void
    {
        if (!app()->runningInConsole()) {
            set_time_limit(config('sparkle.sync.timeout'));
        }

        if (config('sparkle.memory_limit')) {
            ini_set('memory_limit', config('sparkle.memory_limit') . 'M');
        }
    }
