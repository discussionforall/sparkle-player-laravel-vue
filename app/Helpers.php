 ...
 * If this installation of Sparkle has a CDN_URL configured, use it as the base.
...
{
    $cdnUrl = trim(config('sparkle.cdn.url'), '/ ');
...
{
    return $fileName ? public_path(config('sparkle.album_cover_dir') . $fileName) : null;
}

function album_cover_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('sparkle.album_cover_dir') . $fileName) : null;
}

function artist_image_path(?string $fileName): ?string
{
    return $fileName ? public_path(config('sparkle.artist_image_dir') . $fileName) : null;
}

function artist_image_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('sparkle.artist_image_dir') . $fileName) : null;
}

function playlist_cover_path(?string $fileName): ?string
{
    return $fileName ? public_path(config('sparkle.playlist_cover_dir') . $fileName) : null;
}

function playlist_cover_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('sparkle.playlist_cover_dir') . $fileName) : null;
}

function user_avatar_path(?string $fileName): ?string
{
    return $fileName ? public_path(config('sparkle.user_avatar_dir') . $fileName) : null;
}

function user_avatar_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('sparkle.user_avatar_dir') . $fileName) : null;
}

function sparkle_version(): string
{
